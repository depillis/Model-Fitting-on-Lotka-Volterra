...
% there is no need to generate measurement here. 
...
x0_real = y(:,1);
%params0 = [0.62526; 0.6607; 0.1896; 0.0468];%initial guess of parameters

params0 = [0.4; 0.8; 0.018; 0.023]; % alpha, gamma, beta, delta 
                                            % a1, b1, a2, b2

% April 23
% initial values do not match with Mahafy's true parameter values
% If we change initial guess of parameters, do we need to change covariance
% matrix? 
...
prey_error = sum(prey_error(:, 45:91)); % what is 45:91? 
MSE_prey = prey_error / 47;

predator_error = xhat_real(2,:) - x(2,:);
predator_error = predator_error.^2;

predator_error = sum(predator_error(:,45:91)); % why only add from 45 to 91?
