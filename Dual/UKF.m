...
% INPUT: 
%       stateAll: model state variables only 
...
%
% OUTPUT
%       xhat: estimated state variables for the next time step 
%       Phat: updated covariance matrix 
%       negloglik: negative log likelihood measure

function [xhat,Phat,negloglik] = UKF(stateAll, Pstate,Q,R, obsAll,  InferenceDS)
...
negloglik = .5*Odim*log(2*pi)+.5*log(det(Py))+...
    .5*(inov(:,1)'*inv(Py)*inov(:,1));
