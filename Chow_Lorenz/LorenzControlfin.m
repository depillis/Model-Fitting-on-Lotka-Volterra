...
figure(4)
p1=plot(1:T,y(1,:),'k','Linewidth',2);hold on;
p2=plot(1:T,y(2,:),'r-.','Linewidth',2);
p3=plot(1:T,y(3,:),'b+');hold off;
ylabel('Data','fontsize',15);
xlabel('Time','fontsize',15);
legend('x_{1t}','x_{2t}','x_{3t}');
...
% UKF procedure 
R.cov = diag(finalpar); % diag(diag([repmat(10e-4,Ny)])) %
Q.cov=diag(diag([repmat(10e-4,Nx)]));
% what if we update x0 and keep the measurement 
x0(4:6) = finalpar;
InfDS.par = finalpar; 
