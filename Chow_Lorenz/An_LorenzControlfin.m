...
Npar=6;
Nsubj=1;
truepar = [10 28 8/3 26 34 32]; %bifurcation parameters 
%Q = zeros(Nx,Nx);
Q = [];
...
x = zeros(Nx,T); % store parameters & system variables
y = zeros(Ny,T); % store only system variables

% noise to the dynamical species
measureNoise = zeros(Ny,T);
measureNoise(:,1) = [sqrt(truepar(4))*randn(1,1);...
    sqrt(truepar(5))*randn(1,1); sqrt(truepar(6))*randn(1,1)];   

% Initial states of the system variables are the first 3 entries
% why are they negative?
% y(:,1) are the x(:,1) + noise 

x(:,1) = [-5; 20; -5;truepar(1:3)'];   
y(:,1) = feval('LormeasO',x(:,1),[],[],[],[]) + measureNoise(:,1);

%generate "fake data" using true bifurcation parameters 
% to make it more realistic, all they did is to add noise in the model
% variables
% Chow did not use built-in ode function rather write hand code it
for t=2:T,
  measureNoise(:,t) = [sqrt(truepar(4))*randn(1,1);...
      sqrt(truepar(5))*randn(1,1); sqrt(truepar(6))*randn(1,1)];    
...
x0 = [y(:,1);6;20;2]; % initial guess, why the last 3 entries are different from par0?
P0 = diag([repmat(1,6,1)]);% diagonal matrix of entries =1 
par0= x0(4:6);    %[30 40 35];
...
%Jun 3, 2021: An Do 
% in the original code, the par0 has nothing to do with x0
% modified, making par0 =x0(4:6)
...
% Jun 3, 2021
% Without using ML, start with any initial parameters 
% run UKF routine 
% Keep process noise matrix Q.cov=diag(diag([repmat(10e-4,Nx)]));
% measurement noise  matrix is defined as 
R.cov= diag(par0)
[xhat, Px, xh_, Px_, yh_, inov, Py, KG, negloglik,Pxall, Pyall, Px_all]=...
    ukfaddmultiP(x0,P0,Q, R, y,[],[],InfDS);
figure(10); plot(xhat(4:6,:)') % show that parameters do not converge
xhat(4:6,end)

% update process noise and measurement error matrix
% UKF procedure R.cov = diag(finalpar);
...
xhat(4:6,end)
figure(11); plot(xhat(4:6,:)') % show that parameters do not converge
...
figure(5)
plot(xhat(4,:),'k','Linewidth',2); hold on; 
plot(xhat(5,:),'k','Linewidth',2); hold on; 
plot(xhat(6,:),'k','Linewidth',2); hold on; 
hold off
% ylabel('xhat_{3t}','fontsize',24);
% xlabel('xhat_{1t}','fontsize',24);
